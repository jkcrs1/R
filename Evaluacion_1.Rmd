...
grafico1 <- grafico_torta(permiso_muestra,"grupo_vehiculo")
...
IQR_Valor_Pagado <- IQR(vehiculo_dataset$valor_pagado, na.rm = TRUE)
Q1 <- quantile(vehiculo_dataset$valor_pagado, 0.25, na.rm = TRUE)
Q3 <- quantile(vehiculo_dataset$valor_pagado, 0.75, na.rm = TRUE)
lower_bound <- Q1 - 1.5 * IQR_Valor_Pagado
upper_bound <- Q3 + 1.5 * IQR_Valor_Pagado

# Filtrar los valores atípicos
outliers <- vehiculo_dataset %>%
  filter(valor_pagado < lower_bound | valor_pagado > upper_bound)

# Eliminar los valores atípicos
vehiculo_sin_outliers <- vehiculo_dataset %>%
  filter(valor_pagado >= lower_bound & valor_pagado <= upper_bound)
