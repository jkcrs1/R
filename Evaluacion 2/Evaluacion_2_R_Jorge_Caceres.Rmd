...
```{r}
GridSearchCV <- function(data, target_var, parameters, n_folds = 3, scoring = c("RMSE", "MAE")) {
  # Validar los argumentos
  if (!is.data.frame(data)) stop("El argumento 'data' debe ser un data.frame")
  if (!target_var %in% colnames(data)) stop("El argumento 'target_var' debe ser una columna del data.frame")
  ...
  # Identificar y mostrar filas con valores faltantes
  ...
  if (any(colSums(is.na(data)) > 0)) {
    stop("Aún hay valores faltantes en los datos después de eliminar filas con NA")
  }
  ...
  # Eliminar columnas con un solo nivel (un valor único)
  ...
  data[[target_var]] <- droplevels(data[[target_var]])
   ...
   # Convertir niveles de factores a nombres válidos de R
  ...
  summary_function <- if (is_classification) twoClassSummary else defaultSummary
...
result <- GridSearchCV(data, "marca", parameters, n_folds = 5, scoring = "RMSE")
```
